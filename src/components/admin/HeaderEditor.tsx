  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className={cardClass}>
        ...
        {header.animation === 'wrapped3d' && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-5 pt-5 border-t border-[var(--color-border)]">
            <label className="text-xs font-semibold space-y-1 block">Perspectiva
              <input className={controlClass} type="number" min="300" max="2000" step="50" value={header.animationPerspective} onChange={e => update({ animationPerspective: Number(e.target.value) })} />
            </label>
            <label className="text-xs font-semibold space-y-1 block">Profundidade
              <input className={controlClass} type="number" min="0" max="180" step="5" value={header.animationDepth} onChange={e => update({ animationDepth: Number(e.target.value) })} />
            </label>
            <label className="text-xs font-semibold space-y-1 block">Velocidade
              <input className={controlClass} type="number" min="0" max="3" step="0.1" value={header.animationSpeed} onChange={e => update({ animationSpeed: Number(e.target.value) })} />
            </label>
            <label className="text-xs font-semibold space-y-1 block">Mouse
              <input className={controlClass} type="number" min="0" max="3" step="0.1" value={header.animationMouseStrength} onChange={e => update({ animationMouseStrength: Number(e.target.value) })} />
            </label>
            <label className="text-xs font-semibold space-y-1 block">Repetição
